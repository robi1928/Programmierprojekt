...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
    ...
    if (empty($fehler)) {
        try {
            // optional, aber sauberer: beides in einer Transaktion
            if (method_exists($pdo, 'beginTransaction')) {
                $pdo->beginTransaction();
            }

            // Datum ins TIMESTAMP-Format bringen
            $einstellungsdatum_sql = date('Y-m-d H:i:s', $ts);

            // 1. Benutzer in benutzer anlegen
            $stmt = $pdo->prepare("
                INSERT INTO benutzer
                    (vorname, nachname, email, rollen_id, wochenstunden, einstellungsdatum)
                VALUES
                    (:vorname, :nachname, :email, :rollen_id, :wochenstunden, :einstellungsdatum)
            ");
            $stmt->execute([
            ...
            // 2. ID des neuen Benutzers
            $benutzerId = (int)$pdo->lastInsertId();

            // 3. Urlaubskonto für Einstellungsjahr anlegen
            $jahr = (int)date('Y', $ts);
    ...
    <form action="" method="post" class="form">
        ...
        <div class="field">
            ...
            <input type="text" id="vorname" name="vorname" required
                   value="<?= htmlspecialchars($_POST['vorname'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
        </div>
