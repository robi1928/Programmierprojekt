...
// geschrieben von ChatGTP
// stellt sicher, dass für eine Nutzer/Monat/Jahr Kombi exakt ein Stundenzettel existiert
function stundenzettel_check(PDO $pdo, int $benutzerId, int $monat, int $jahr): int {
...
// selber geschrieben mit stackoverflow. Vorgeschlagen von GTP
// Berechnet und updated die ist_stunden bei jedem insert/update/delete neu
...
// verbessert mit GTP
// normaliisiert alle eingetragenen Werte
if ($_SERVER['REQUEST_METHOD'] === 'POST') { // nach formularversand holt es sich die Werte und setzt sonst leeren String
  $datum = $_POST['datum'] ?? '';
  ...
  try {
    // prüft Datum. Die eigentliche Browser Fehlermeldung ist nicht in allen Browsern.
    ...
    if ($status === 'krank' || $status === 'urlaub') {
      $stunden = 0.0;
      $bemerkung = $status;
      // Ort spielt bei Krankheit/Urlaub keine Rolle
      $ortId = 0;
    } else {
      // Zeiten prüfen
      ...
      // Ort prüfen (bestenfalls irrelevant, weil das auswahlmenü funktioniert)
      $validOrt = array_column($orte, 'ort_id');
      if (!in_array($ortId, $validOrt, true)) throw new RuntimeException('Ungültiger Arbeitsort.');
    }

    // Stundenzettel prüfen und falls notwendig anlegne
    $d = new DateTimeImmutable($datum);
    $szId = stundenzettel_check($pdo, (int)$benutzer['id'], (int)$d->format('n'), (int)$d->format('Y'));
    $tag = (int)$d->format('j');

    // Aktualisiert Zeiteintrag, erstellt keinen neuen
    $stmt = $pdo->prepare("
...
<!-- darstellung von Fehler& Erfolgsmeldungen.-->
  ...
  <form class="form" method="post" autocomplete="off">
    ...
    <!-- zweiter Versuch. Erstmal so lassen, bis eine bessere Darstellung einfällt-->
    <div class="status">
      ...
      <label class="status-option krank">
        <input type="radio" name="status" value="krank">
        Krank
      </label>
      <label class="status-option urlaub">
        <input type="radio" name="status" value="urlaub">
        Urlaub
      </label>
    </div>
    ...
    <div id="zeitBlock">
      ...
      <div class="field">
        ...
        <input id="bemerkung" name="bemerkung" type="text" maxlength="500" placeholder="z. B. Details">
      </div>
    </div>
    ...
    <div>
      ...
      <a class="btn" href="?">Zurücksetzen</a>
        ...
        <!-- besser und woanders unterbringen-->
