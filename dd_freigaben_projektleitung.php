...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($typ === 'stundenzettel') {
            CStundenzettelRepository::freigabeDurchProjektleitung(
                $pdo,
                $id,
                $aktuellerBenutzerId,
                $entscheidung
            ...
            CUrlaubsantragRepository::freigabeDurchProjektleitung(
                $pdo,
                $id,
                $aktuellerBenutzerId,
                $entscheidung,
...
// Offene Freigaben (Stundenzettel) – eingereicht von TL oder Mitarbeitern
$sqlStz = "
    SELECT
        sz.stundenzettel_id,
        sz.monat,
    ...
    JOIN benutzer b_besitzer
      ON b_besitzer.benutzer_id = sz.benutzer_id
    JOIN benutzer e
      ON e.benutzer_id = sz.eingereicht_von
    JOIN rollen r_b
      ON r_b.rollen_id = b_besitzer.rollen_id
    JOIN rollen r_e
      ON r_e.rollen_id = e.rollen_id
    WHERE sz.status        = 'entwurf'
...
// Offene Freigaben (Urlaub) – eingereicht von TL oder Mitarbeitern
$sqlUrlaubOffen = "
    SELECT
        a.antrag_id,
        a.start_datum,
    ...
    JOIN rollen r_a
      ON r_a.rollen_id = b_antragsteller.rollen_id
    JOIN rollen r_e
      ON r_e.rollen_id = e.rollen_id
    WHERE a.status        = 'entwurf'
      AND a.eingereicht_am IS NOT NULL
      AND r_e.rollen_schluessel IN ('Teamleitung', 'Mitarbeiter')
    ORDER BY a.start_datum
";
$stmtUrlaubOffen = $pdo->prepare($sqlUrlaubOffen);
$stmtUrlaubOffen->execute();
$offeneUrlaube = $stmtUrlaubOffen->fetchAll(PDO::FETCH_ASSOC);

// Abgelehnte Urlaubsanträge für diese Projektleitung (eigene Anträge)
$sqlUrlaubAbgelehnt = "
    SELECT
        a.antrag_id,
        a.start_datum,
